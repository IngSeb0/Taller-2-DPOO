...
public class SandboxListas
{
    ...
    {
    	listaCadenas.removeIf(cad -> cad.equals(cadena));
    ...
    {
    	int count = 0;
        List<Integer> enterosContados = new ArrayList<>();

        for (int i = 0; i < listaEnteros.size(); i++) {
            int num = listaEnteros.get(i);
            if (!enterosContados.contains(num)) {
                for (int j = i + 1; j < listaEnteros.size(); j++) {
                    if (num == listaEnteros.get(j)) {
                        count++;
                        enterosContados.add(num);
                        break;
                    }
                }
            }
